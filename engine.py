...
def download_url(url: str, job_dir: Path) -> Path:
    ...
    if not url.startswith(("http://", "https://")):
        raise ValueError("The link must begin with http:// or https://")
    try:
        import yt_dlp
    ...
    opts = {"outtmpl": destination, "quiet": True, "noplaylist": True, "max_filesize": maximum,
            "format": "mp4[height<=1080]/mp4/best[height<=1080]/best"}
    try:
        with yt_dlp.YoutubeDL(opts) as ydl:
            info = ydl.extract_info(url, download=True)
            path = Path(ydl.prepare_filename(info))
    except Exception as exc:
        raise RuntimeError(f"Link download failed ({type(exc).__name__}): {exc}") from exc
    if not path.exists():
        candidates = sorted(job_dir.glob("linked-source.*"))
        if not candidates: raise RuntimeError("The site reported a download but no video file was created.")
        path = candidates[0]
    if path.stat().st_size > maximum: raise RuntimeError("Downloaded video exceeds the configured server limit.")
...
def _frames(path: Path, count: int = 12) -> tuple[list[np.ndarray], float]:
    ...
    if not cap.isOpened(): raise RuntimeError("Cannot open this video. Send an MP4/H.264 file or a different public URL.")
    fps = cap.get(cv2.CAP_PROP_FPS) or 24
    total = int(cap.get(cv2.CAP_PROP_FRAME_COUNT) or 0)
    duration = total / fps if total else 0
    if duration <= 0 or duration > 15 * 60: raise RuntimeError("Video duration could not be read or is longer than 15 minutes.")
    ...
    cap.release()
    if not result: raise RuntimeError("No readable frames found in the video.")
...
Do not infer a full-match score from a short highlight. The clip duration is %s seconds. User's untrusted hint: %s""" % (round(duration,1), hint or "none")
    ...
    try:
        ...
        raise RuntimeError(f"Vision analysis failed: {type(exc).__name__}: {exc}") from exc
    goals = []
...
def _font(size: int) -> ImageFont.ImageFont:
    candidates = [r"C:\Windows\Fonts\arialbd.ttf", r"C:\Windows\Fonts\impact.ttf", "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf"]
...
def _logo(path: Path, size: int = 230) -> Image.Image:
    ...
    except Exception as exc: raise RuntimeError(f"Logo cannot be opened ({path.name}). Send PNG, JPG, or WebP.") from exc
...
def make_overlay(source: Path, logo_home: Path, logo_away: Path, facts: MatchFacts, job_dir: Path) -> Path:
    ...
    if not writer.isOpened(): raise RuntimeError("MP4 writer is unavailable. Install OpenCV/FFmpeg and restart the bot.")
    ...
    writer.release()
    if not output.exists() or output.stat().st_size < 1024: raise RuntimeError("Overlay render did not produce a usable MP4.")
    return output
